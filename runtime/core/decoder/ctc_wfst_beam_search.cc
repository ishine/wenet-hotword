...
void CtcWfstBeamSearch::Search(const std::vector<std::vector<float>>& logp) {
  ...
  for (int i = 0; i < logp.size(); i++) {
    ...
    if (blank_score > opts_.blank_skip_thresh * opts_.blank_scale) {
      ...
      // LOG(INFO) << "Active frame " << num_frames_ 
      //       << ": best_id=" << cur_best 
      //       << " (blank_prob=" << std::exp(logp[i][0])
      //       << ", best_non_blank_prob=" << std::exp(logp[i][cur_best]) << ")";
  ...
  if (decoded_frames_mapping_.size() > 0) {
    ...
    for (int id : outputs_[0]) {
      debug_output += std::to_string(id) + " ";
    }
    ...
    LOG(WARNING) << "Empty decoded result! Possible causes:\n";
  }
}
