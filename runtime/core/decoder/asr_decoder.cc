...
bool AsrDecoder::TextToIds(const std::string& text, const std::vector<WordPiece> word_pieces, std::vector<int>* ids) {
  ids->clear();
  ...
  for (size_t i = 0; i < chars.size(); ++i) { 
    ...
    if (i < word_pieces.size()) {
        ...
        if (tpl_token.size() >= 4 && tpl_token[0] == '\xe2' && tpl_token[1] == '\x96' && tpl_token[2] == '\x81') {
    ...
    if (id == -1 && oov_mapping_ != nullptr) {
      ...
      if (it != oov_mapping_->end()) {
        ...
        if (proxy_id != -1) {
          id = proxy_id;
          VLOG(2) << "Proxy Mapping: " << ch << " -> " << proxy_ch << " (ID: " << id << ")";
        } else {
          LOG(WARNING) << "Proxy char " << proxy_ch << " for " << ch << " is also OOV!";
        }
      }
    }
...
float AsrDecoder::CalculateMatchBonus(const HotwordCorrection::MatchResult& match) {
    std::vector<std::string> chars;
    wenet::SplitUTF8StringToChars(match.hotword, &chars);
    int char_len = chars.size();
    float length_factor = std::log2(std::max(2, char_len));

    float base_bonus = 2.0f * match.score * length_factor;
    float safe_divisor = std::max(0.4f, match.avg_confidence);
    float bonus = base_bonus / safe_divisor;

    return bonus;
}

std::string AsrDecoder::ApplyMatchesToSentence(const std::string& original, 
                                   std::vector<HotwordCorrection::MatchResult> matches) {
    if (matches.empty()) return original;

    std::sort(matches.begin(), matches.end(), 
        [](const HotwordCorrection::MatchResult& a, const HotwordCorrection::MatchResult& b) {
...
// if (!result_.empty() && symbol_table_ != nullptr && !global_ctc_log_probs_.empty()) {
//     int num_frames = global_ctc_log_probs_.size();
//     // Grid[FrameIdx] -> map<TokenID, Count>
//     std::vector<std::unordered_map<int, int>> frame_grid(num_frames);
    
//     const auto& all_outputs = searcher_->Outputs();
//     const auto& all_times = searcher_->Times();
//     int total_paths = all_outputs.size();
//     if (total_paths == 0) return; 
//     for (size_t i = 0; i < all_outputs.size(); ++i) {
//       const auto& outputs = all_outputs[i]; 
//       const auto& times = all_times[i];
//       for (size_t j = 0; j < outputs.size(); ++j) {
//         int token_id = outputs[j]; 
//         int frame_idx = times[j];
//         if (frame_idx >= 0 && frame_idx < num_frames) {
//           frame_grid[frame_idx][token_id]++;
//         }
//       }
//     }
//     DecodeResult consensus_path;
//     consensus_path.score = result_[0].score + 1; 
//     int last_token_id = -1;
//     static std::mt19937 generator(std::random_device{}());
//     std::uniform_real_distribution<float> distribution(0.0f, 1.0f);
//     for (int f = 0; f < num_frames; ++f) {
//       int count_in_frame = 0;
//       for (const auto& kv : frame_grid[f]) {
//         count_in_frame += kv.second;
//       }
//       float prob_blank = static_cast<float>(total_paths - count_in_frame) / total_paths;
//       float rand_val = distribution(generator);
      
//       int selected_token_id = -1;
//       if (rand_val < prob_blank) {
//         continue;
//       }
//       float accum_prob = prob_blank; 
      
//       for (const auto& kv : frame_grid[f]) {
//         float token_prob = static_cast<float>(kv.second) / total_paths;
//         accum_prob += token_prob;
        
//         if (rand_val < accum_prob) {
//           selected_token_id = kv.first;
//           break;
//         }
//       }
//       if (selected_token_id != -1 && selected_token_id != last_token_id) {
//         std::string word = symbol_table_->Find(selected_token_id);
        
//         consensus_path.sentence += (' ' + word);
        
//         last_token_id = selected_token_id;
//       }
//     }
//     if (!consensus_path.sentence.empty()) {
//       bool exists = std::any_of(result_.begin(), result_.end(), [&](const DecodeResult& r) {
//           return r.sentence == consensus_path.sentence;
//       });
      
//       if (!exists) {
//         result_.push_back(consensus_path);
//         VLOG(3) << "Generated Consensus Path (Sampling): " << consensus_path.sentence;
//       }
//     }
//   }
  ...
  if (corrector_ != nullptr && !result_.empty()) {
      ...
      for (const auto& m : all_matches) {
        VLOG(2) << "current path: " << path.sentence << "\n matches: " << m.hotword << " start: " << m.start << " end: " << m.end << " score: " << m.score << " avg_conf: " << m.avg_confidence;
      } 
      ...
      // =========================================================
      {
          ...
          if (generated_sentences.find(path_full.sentence) == generated_sentences.end()) {
        ...
        // =========================================================
        {
            ...
            if (!robust_matches.empty() && robust_matches.size() < all_matches.size()) {
                DecodeResult path_robust = path;
                path_robust.sentence = ApplyMatchesToSentence(path.sentence, robust_matches);
                ...
                if (generated_sentences.find(path_robust.sentence) == generated_sentences.end()) {
        ...
        // =========================================================
        {
            auto best_match_iter = std::max_element(all_matches.begin(), all_matches.end(),
                [](const auto& a, const auto& b) { return a.score < b.score; });

            if (best_match_iter != all_matches.end()) {
                DecodeResult path_top1 = path;
                path_top1.sentence = ApplyMatchesToSentence(path.sentence, {*best_match_iter});
                path_top1.score += CalculateMatchBonus(*best_match_iter);
                path_top1.corrected_hotwords.push_back(best_match_iter->hotword);
                if (generated_sentences.find(path_top1.sentence) == generated_sentences.end()) {
    ...
    if (!new_candidates.empty()) {
        ...
        result_.insert(result_.end(), new_candidates.begin(), new_candidates.end());
    }
  }
  ...
  void AsrDecoder::UpdateResult(bool finish) {
    ...
    for (size_t i = 0; i < hypotheses.size(); i++) {
      ...
      if (unit_table_ != nullptr && finish) {
        ...
        for (size_t j = 0; j < input.size(); j++) {
          ...
          if (frame_idx >= 0 && frame_idx < global_ctc_log_probs_.size()) {
            ...
            if (ctc_idx >= 0 && ctc_idx < global_ctc_log_probs_[frame_idx].size()) {
      ...
      // debug 3
      // if (searcher_->Type() == kWfstBeamSearch && !path.sentence.empty()) {
      //   if (path.sentence.front() == ' ') {
      //     LOG(WARNING) << "WFST leading space: '" << path.sentence << "'";
      //   }
      // }
    ...
    //     VLOG(1) << "Pointer address: " << context_graph_.get();
    //     if (context_graph_ != nullptr and !first_token_padding_flag) {
    //       int cur_state = 0;
    //       float score = 0;
    //       for (int ilabel : inputs[0]) {
    //         VLOG(1) << "context_graph input hanzi ilabel: " << ilabel;


    //         cur_state = context_graph_->GetNextState(cur_state, ilabel, &score,
    //                                                 &(result_[0].contexts));
    //         //cur_state = context_graph_->GetNextState(cur_state, ilabel, &score,&result_[0].score,
    //         //                                         &(result_[0].contexts));
    //       }
    //       // result_[0].score = result_[0].score + score;
    //       std::string contexts;
    ...
    if(!finish){
      ...
      if (context_hanzi_graph_ != nullptr) {
        ...
        float score = 0;
        for (int ilabel : inputs[0]) {
          cur_state = context_hanzi_graph_->GetNextState(cur_state, ilabel, &score,
                                                  &(result_[0].contexts));
        }
        std::string contexts;
        for (const auto& context : result_[0].contexts) {
  ...
  void AsrDecoder::Rescoring() {
    ...
    for (size_t i = num_original_paths; i < num_paths; ++i) {
      ...
      if (success) {
        ...
        LOG(WARNING) << "Failed to convert text to ids (should not happen): " << result_[i].sentence; 
      }
    }
    ...
    for (int i = 0; i < result_.size(); i++) {
      VLOG(2) << "Final Rank " << i << ": " << result_[i].sentence << " Score:" << result_[i].score;
    }
  }
