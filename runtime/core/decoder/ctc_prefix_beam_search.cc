    ...
    for (int i = 0; i < topk_index.size(); ++i) {
      ...
      for (const auto& it : cur_hyps_) {
        ...
        if (id == opts_.blank) {
          ...
          LOG(INFO) << "Case 0: Extend blank, prefix=" << VectorToString(prefix)
            << ", prob=" << prob << ", update s-score=" << next_score.s;
          ...
          // LOG(INFO) << "Case 1: Repeat token, prefix=" << VectorToString(prefix)
          //   << ", token=" << id << ", prob=" << prob 
          //   << ", update ns-score=" << next_score1.ns;
          ...
          // LOG(INFO) << "Case 2: New prefix=" << VectorToString(new_prefix)
          // << ", prob=" << prob << ", update ns-score=" << next_score2.ns;
          ...
          // LOG(INFO) << "Case 3: New prefix=" << VectorToString(new_prefix)
          // << ", prob=" << prob << ", update ns-score=" << next_score.ns;
        }
      }
    }
...
void CtcPrefixBeamSearch::FinalizeSearch() {
  ...
  LOG(INFO) << "FinalizeSearch: hypotheses_.size()=" << hypotheses_.size()
          << ", context_graph_ enabled? " << (context_graph_ != nullptr);
  LOG(INFO) << "enter FinalizeSearch";
  ...
  for (const auto& prefix : hypotheses_) {
    ...
    LOG(INFO) << "Prefix: " << VectorToString(prefix)
            << ", context_state=" << cur_hyps_[prefix].context_state;

    if (prefix_score.context_state != 0) {
      LOG(INFO) << "Backoff context: prefix=" << VectorToString(prefix)
          << ", state=" << prefix_score.context_state
          << ", before context_score=" << prefix_score.context_score;

      prefix_score.UpdateContext(context_graph_, prefix_score, -1);

      LOG(INFO) << "After backoff: context_score=" << prefix_score.context_score;
    }
  }
