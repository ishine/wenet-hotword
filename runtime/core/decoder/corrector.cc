...
const std::map<std::pair<std::string, std::string>, float> CONFUSION_MATRIX = {
    {{"z", "zh"}, 0.1f},     {{"zh", "z"}, 0.1f},     {{"c", "ch"}, 0.1f},
    {{"ch", "c"}, 0.1f},     {{"s", "sh"}, 0.1f},     {{"sh", "s"}, 0.1f},

    {{"l", "n"}, 0.2f},      {{"n", "l"}, 0.2f},      {{"f", "h"}, 0.3f},
    {{"h", "f"}, 0.3f},      {{"an", "ang"}, 0.2f},   {{"ang", "an"}, 0.2f},
    {{"en", "eng"}, 0.2f},   {{"eng", "en"}, 0.2f},   {{"in", "ing"}, 0.2f},
    {{"ing", "in"}, 0.2f},

    {{"ian", "iang"}, 0.2f}, {{"iang", "ian"}, 0.2f}, {{"uan", "uang"}, 0.2f},
    {{"uang", "uan"}, 0.2f},
};

// =====================  Dense matrix runtime state  =====================
// Populated by LoadConfusionMatrix(). When dense matrix is loaded,
// _get_tuple_cost and PhonemeIndex::get_candidates dispatch to it instead of
// the sparse fallback.
namespace {
bool g_dense_loaded = false;
std::map<std::pair<std::string, std::string>, float> g_dense_matrix;
std::unordered_map<std::string, std::vector<std::string>> g_dense_neighbors;
...
const std::unordered_set<std::string>& initial_set() {
  static const std::unordered_set<std::string> s(
      PinyinSplitter::INITIALS.begin(), PinyinSplitter::INITIALS.end());
  return s;
}

PhonemeClass classify_zh(const std::string& p) {
  if (p.empty()) return PhonemeClass::kOther;
  if (p.size() == 1 && std::isdigit(static_cast<unsigned char>(p[0])))
    return PhonemeClass::kTone;
  if (initial_set().count(p)) return PhonemeClass::kInitial;
  return PhonemeClass::kFinal;
}
...
void LoadConfusionMatrix(const std::string& path) {
  g_dense_matrix.clear();
  g_dense_neighbors.clear();
  g_dense_loaded = false;
  if (path.empty()) {
    LOG(INFO) << "ConfusionMatrix: no path given; using sparse fallback ("
              << CONFUSION_MATRIX.size() << " entries).";
    return;
  }
  std::ifstream f(path);
  if (!f.is_open()) {
    LOG(WARNING) << "ConfusionMatrix: cannot open " << path
                 << "; using sparse fallback.";
    return;
  }
  std::string line;
  int n = 0, skipped = 0;
  while (std::getline(f, line)) {
    ...
    g_dense_matrix[{a, b}] = cost;
    if (cost < g_neighbor_threshold && a != b) {
      g_dense_neighbors[a].push_back(b);
    }
    ++n;
  }
  g_dense_loaded = (n > 0);
  LOG(INFO) << "ConfusionMatrix: loaded " << n << " entries from " << path
            << " (skipped " << skipped << "); neighbors for "
            << g_dense_neighbors.size() << " phonemes (threshold "
            << g_neighbor_threshold << ").";
}
// =====================  End dense matrix state  =====================

float CalculateAvgConfidenceInRange(const std::string& text,
...
SplitPhoneme PinyinSplitter::split(const std::string& hanzi,
                                   const std::string& tone3) {
  SplitPhoneme p;
  p.hanzi = hanzi;
  ...
  if ((p.initial == "j" || p.initial == "q" || p.initial == "x" ||
       p.initial == "y") &&
      (p.final == "v" || p.final == "yu")) {
    p.final = "u";
  }
...
size_t PinyinProvider::process_zh(const std::string& text, size_t pos,
  ...
  ::Pinyin::PinyinResVector res_tones;
  {
    ...
    res_tones =
        g_pinyin->hanziToPinyin(fragment, ::Pinyin::ManTone::Style::TONE3,
                                ::Pinyin::Error::Default, false, false, true);
  }
  ...
  for (const auto& item : res_tones) {
    ...
    if (!sp.is_valid_pinyin) {
      ...
      if (!sp.initial.empty()) {
        seq.push_back({sp.initial, Lang::ZH, true, false, start, end});
        has_added = true;
      }
      if (!sp.final.empty()) {
        seq.push_back({sp.final, Lang::ZH, !has_added, false, start, end});
      }
      if (!sp.tone.empty()) {
        seq.push_back({sp.tone, Lang::ZH, false, true, start, end});
      } else if (!seq.empty())
        seq.back().is_word_end = true;
    }
  }
  return scan_pos;
}
...
void EnglishProvider::initialize(const std::string& dict_path) {
  std::call_once(init_flag, [&]() {
    ...
    LOG(INFO) << "EnglishProvider loaded " << count << " words from "
              << dict_path;
  });
}

bool EnglishProvider::get_phonemes(const std::string& word,
...
size_t process_en_num(const std::string& text, size_t pos,
                      std::vector<Phoneme>& seq, bool split_char) {
  ...
  bool is_alpha = std::all_of(token.begin(), token.end(),
  ...
  if (is_alpha && EnglishProvider::get_phonemes(token, arpabet_phonemes)) {
    for (size_t i = 0; i < arpabet_phonemes.size(); ++i) {
      seq.push_back({arpabet_phonemes[i], Lang::EN, (i == 0),
                     (i == arpabet_phonemes.size() - 1), start_pos, end_pos});
    }
  } else if (is_alpha) {
    for (size_t i = 0; i < token.length(); ++i) {
      ...
      if (EnglishProvider::get_phonemes(letter_str, letter_phones)) {
        for (const auto& ph : letter_phones) {
          seq.push_back({ph, Lang::EN, (i == 0), (i == token.length() - 1),
        ...
        std::transform(lower_char.begin(), lower_char.end(), lower_char.begin(),
                       ::tolower);
        seq.push_back({lower_char, Lang::EN, (i == 0),
                       (i == token.length() - 1), start_pos + i,
                       start_pos + i + 1});
      }
    }
  } else {
    ...
    std::transform(token_lower.begin(), token_lower.end(), token_lower.begin(),
                   ::tolower);
    Lang lang = (!token_lower.empty() && std::isdigit(token_lower[0]))
                    ? Lang::NUM
                    : Lang::EN;

    if (split_char) {
      for (size_t i = 0; i < token.length(); ++i) {
        seq.push_back({std::string(1, token_lower[i]), lang, (i == 0),
                       (i == token.length() - 1), start_pos + i,
                       start_pos + i + 1});
      }
    } else {
...
std::vector<Phoneme> get_phoneme_info(const std::string& text) {
  ...
  while (pos < len) {
    ...
    if (char_len == 3 && c >= 0xE4 && c <= 0xE9) {
      ...
      LOG(INFO) << "Found unrecognizable char in text: " << text
                << " , skipping this word.";
      return {};
    }
  }
...
float _get_tuple_cost(const Phoneme& t1, const Phoneme& t2) {
  ...
  if (t1.lang == Lang::ZH) {
    ...
    if (g_dense_loaded) {
      // Class-aware: cross-class (initial vs final) is never a valid
      // substitution; only same-class pairs are looked up.
      auto c1 = classify_zh(t1.value);
      auto c2 = classify_zh(t2.value);
      if (c1 != c2) return 1.0f;
      auto it = g_dense_matrix.find(key);
      if (it != g_dense_matrix.end()) return it->second;
      return 1.0f;
    }
  ...
  if (t1.lang == Lang::EN) {
    ...
    if (max_len > 0)
      return 1.0f - (static_cast<float>(Utils::lcs_length(t1.value, t2.value)) /
                     max_len);
  }
  return 1.0f;
}
...
std::vector<std::tuple<float, int, int>> fuzzy_substring_search_constrained(
    const std::vector<Phoneme>& hw_info, const std::vector<Phoneme>& input_info,
    float threshold) {
  int n = hw_info.size();
  int m = input_info.size();
  if (n == 0 || m == 0) return {};
  std::vector<std::vector<float>> dp(
      n + 1, std::vector<float>(m + 1, std::numeric_limits<float>::infinity()));
  ...
  for (int j = 0; j <= m; ++j) {
    ...
    if (j < m && input_info[j].is_word_start) {
      dp[0][j] = 0.0f;
      path_start[0][j] = j;
    }
  }
  ...
  for (int i = 1; i <= n; ++i) {
    for (int j = 1; j <= m; ++j) {
      ...
      dp[i][j] = min_dist;
      if (min_dist == d_m)
        path_start[i][j] = path_start[i - 1][j - 1];
      else if (min_dist == d_d)
        path_start[i][j] = path_start[i - 1][j];
      else
        path_start[i][j] = path_start[i][j - 1];
    }
  }
  ...
  for (int j = 1; j <= m; ++j) {
    ...
    if (score >= threshold) {
      ...
      if (used_ends.find(j) == used_ends.end() ||
          score > std::get<0>(used_ends[j])) {
...
fuzzy_substring_search_constrained_with_confidence(
    const std::vector<Phoneme>& hw_info, const std::vector<Phoneme>& input_info,
    float threshold, const std::vector<float>& token_confidences,
  ...
  for (int j = 1; j <= m; ++j) {
    ...
    if (score >= threshold) {
      int start_idx = path_start[n][j];
      if (hw_lang == Lang::EN) {
        if (start_idx > 0 && input_info[start_idx - 1].lang == Lang::EN)
          continue;
...
int PhonemeEncoder::encode(const std::string& p) {
  if (phoneme_to_code.find(p) == phoneme_to_code.end()) {
    phoneme_to_code[p] = next_code++;
  }
  return phoneme_to_code[p];
}
std::vector<int> PhonemeEncoder::encode_sequence(
    const std::vector<Phoneme>& phonemes) {
  std::vector<int> res;
  for (const auto& p : phonemes) res.push_back(encode(p.value));
  return res;
}

// PhonemeIndex 实现
void PhonemeIndex::add(const std::string& hotword,
                       const std::vector<Phoneme>& phonemes) {
  if (phonemes.empty()) return;
  std::vector<int> codes = encoder.encode_sequence(phonemes);
  std::vector<int> indices = {0};
  if (phonemes[0].lang == Lang::EN) {
    size_t limit = std::min(codes.size(), (size_t)2);
    for (size_t k = 0; k < limit; ++k) indices.push_back(k);
  }
  for (int idx : indices)
    if (idx < (int)codes.size()) index[codes[idx]].push_back({hotword, codes});
}

std::vector<std::pair<std::string, std::vector<int>>>
PhonemeIndex::get_candidates(const std::vector<Phoneme>& input_phonemes) {
  ...
  for (const auto& p : input_phonemes) {
    ...
    if (p.lang == Lang::ZH) {
      if (g_dense_loaded) {
        auto it = g_dense_neighbors.find(p.value);
        if (it != g_dense_neighbors.end()) {
          for (const auto& nb : it->second) {
            input_codes.insert(encoder.encode(nb));
          }
        }
      } else {
...
std::vector<int> PhonemeIndex::encode_input(const std::vector<Phoneme>& ps) {
  return encoder.encode_sequence(ps);
}

float FastRAG::_distance(const std::vector<int>& main,
                         const std::vector<int>& sub) {
  int n = sub.size(), m = main.size();
  if (n == 0 || m == 0) return (float)n;
  ...
  for (int i = 1; i <= n; ++i)
    for (int j = 1; j <= m; ++j) {
      ...
      dp[i][j] = std::min(
          {dp[i - 1][j] + 1.0f, dp[i][j - 1] + 1.0f, dp[i - 1][j - 1] + cost});
    }
  float min_val = std::numeric_limits<float>::infinity();
  for (int j = 1; j <= m; ++j)
    if (dp[n][j] < min_val) min_val = dp[n][j];
  return min_val;
}

FastRAG::FastRAG(float thr) : threshold(thr) {}
void FastRAG::add_hotwords(
    const std::unordered_map<std::string, std::vector<Phoneme>>& hotwords) {
  index = PhonemeIndex();
  for (const auto& kv : hotwords) index.add(kv.first, kv.second);
}
std::vector<std::pair<std::string, float>> FastRAG::search(
    const std::vector<Phoneme>& input_phonemes, int top_k) {
  ...
  for (const auto& cand : candidates) {
    ...
    float score =
        1.0f - (_distance(input_codes, cand.second) / cand.second.size());
...
// PhonemeCorrector 实现
PhonemeCorrector::PhonemeCorrector(float threshold, float threshold_en)
    : threshold(threshold), threshold_en(threshold_en) {
  this->similar_threshold = threshold - 0.2f;
  fast_rag =
      std::make_unique<FastRAG>(std::min(threshold, similar_threshold) - 0.1f);
}
...
void PhonemeCorrector::update_dynamic_hotwords(
    const std::unordered_map<std::string, float>& word_boost_map) {
  std::lock_guard<std::mutex> guard(_lock);
  dynamic_hotword_boosts_ = word_boost_map;
}

CorrectionResult PhonemeCorrector::correct_with_confidence(
    const std::string& text, const std::vector<float>& token_confidences) {
  ...
  for (const auto& item : fast_results) {
    ...
    auto found = fuzzy_substring_search_constrained_with_confidence(
        hotwords[item.first], input_phonemes, similar_threshold - 0.1f,
        token_confidences);
    ...
    for (const auto& seg : found) {
      ...
      if (score >= current_threshold) {
        float avg_conf = CalculateAvgConfidenceInRange(text, token_confidences,
                                                       c_start, c_end);
...
CorrectionResult PhonemeCorrector::correct(const std::string& text) {
  ...
  for (const auto& item : fast_results) {
    auto found = fuzzy_substring_search_constrained(
        hotwords[item.first], input_phonemes, similar_threshold - 0.1f);
