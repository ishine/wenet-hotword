...
DEFINE_int32(max_append_path, 20, "max number path to append");
...
std::shared_ptr<DecodeResource> InitDecodeResourceFromFlags() {
  ...
  resource->oov_mapping = std::make_shared<std::unordered_map<std::string, std::string>>();
  if (!FLAGS_oov_mapping_path.empty()) {
        LOG(INFO) << "Reading OOV mapping from " << FLAGS_oov_mapping_path;
        std::ifstream infile(FLAGS_oov_mapping_path);
        if (!infile.is_open()) {
            LOG(FATAL) << "Failed to open OOV mapping file: " << FLAGS_oov_mapping_path;
        }
        std::string line;
        int count = 0;
        while (std::getline(infile, line)) {
            ...
            std::string oov_word, proxy_word;
            if (ss >> oov_word >> proxy_word) {
                (*resource->oov_mapping)[oov_word] = proxy_word;
                count++;
            }
        }
        LOG(INFO) << "Loaded " << count << " OOV mappings.";
  }

  // Build Hanzi-based graph for decoding
  if (!FLAGS_context_hanzi_path.empty()) {
    LOG(INFO) << "Reading context " << FLAGS_context_hanzi_path;
    std::vector<std::string> contexts;
    std::ifstream infile(FLAGS_context_hanzi_path);
    std::string context;
  ...
  if (!FLAGS_hotword_path.empty() && !FLAGS_pinyin_dict_path.empty()) {
      ...
      if (!FLAGS_cmu_dict_path.empty()) {
          ...
          LOG(INFO) << "Initializing English G2P Done.";
      } else {
          LOG(WARNING) << "No cmu_dict_path provided. English G2P will fallback to char-split.";
      }

      auto corrector = std::make_shared<HotwordCorrection::PhonemeCorrector>(FLAGS_fuzzy_threshold, FLAGS_fuzzy_threshold_en);
      ...
      if (hw_file.is_open()) {
          ...
          LOG(WARNING) << "Failed to open hotword file: " << FLAGS_hotword_path;
      }
