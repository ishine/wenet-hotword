...
| F_autotune | E_cache + Optuna TPE 调参后的 knee 配置 | 覆盖 12 个参数（见「多目标自动调参」） |
...
| F_autotune | 53 | 6.32 | 90.07 | 96.21 | 93.04 | 254 | 9.00 |
...
**指标变化**：recall 36.17% → 90.07%，CER 12.04% → 6.32%。

Optuna TPE 联合调整 decode + hotword 共 12 个参数，将默认锚点移动到更优的联合工作点。详见「多目标自动调参」节。
...
| 参数 | 默认值 | Knee | 搜索范围 |
|------|-------:|-----:|----------|
| `decode.rescoring_weight` | 1.00 | 0.322 | [0.3, 2.5] |
| `decode.ctc_weight` | 0.50 | 0.788 | [0.2, 0.8] |
| `decode.length_penalty` | 0.00 | -0.142 | [-1.0, 1.0] |
| `decode.nbest` | 10 | 10 | {5, 10, 20} |
| `hotword.fuzzy_threshold` | 0.50 | 0.430 | [0.2, 0.7] |
| `hotword.max_append_path` | 20 | 20 | {10, 20} |
| `hotword.fuzzy_reject_ratio` | 0.8 | 0.720 | [0.5, 0.95] |
| `hotword.confidence_weight_min` | 0.2 | 0.268 | [0.1, 0.5] |
| `hotword.bonus_length_scale` | 0.5 | 0.704 | [0.2, 0.8] |
| `hotword.bonus_weight` | 2.0 | 3.650 | [0.5, 4.0] |
| `hotword.neighbor_threshold` | 0.5 | 0.542 | [0.3, 0.7] |
| `hotword.confidence_floor` | 0.4 | 0.785 | [0.2, 0.8] |
...
| aishell_test (tune, 235) | F_autotune | 6.32 | 90.07 | 96.21 | 93.04 |
| I_indep (held-out, 115) | D_confidence | 11.88 | 48.15 | 98.48 | 64.68 |
| I_indep (held-out, 115) | F_autotune | 7.33 | 91.11 | 98.40 | 94.62 |

Tune set：+40.05 pp F1，−5.72 pp CER。Held-out：+29.94 pp F1，−4.55 pp CER。
...
| F_autotune | 7.33 | 91.11 | 98.40 | 94.62 | 123 | 135 | 125 |
...
| F_autotune | 6.32 / 90.07 | 2.02 / 95.74 |

multi_cn 为 unidirectional decoder（11008 units）。u2pp autotune 共 138 trials（含 Pareto stagnation early exit，patience=40）。
